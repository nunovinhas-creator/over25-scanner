...
class TestUpdateClosingOdds:
    ...
    def test_too_early_no_error_yet(self):
        ...
        picks = [_pick(id="2_home_sh", data=_iso(0.1))]
    ...
    def test_window_closed_marks_terminal_error_once(self):
        """Além de CLOSE_MAX_H (24h) sem odds_fecho → erro terminal explícito,
        marcado uma única vez (não escreve de novo em corridas seguintes)."""
        picks = [_pick(id="5_home_sh", data=_iso(30.0))]
        out = _run(picks, fetch_closing_odds_side_effect=lambda eid, out_: (_ for _ in ()).throw(
